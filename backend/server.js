...
const normalizeKitchenData = (kitchen) => {
 ...
 const plain = kitchen.toObject
  ? kitchen.toObject()
  : kitchen;
 return {
  ...plain,
  name:
   plain.kitchenName || plain.name || "Kitchen",
  description:
   plain.description ||
   plain.bio ||
  ...
  preference: normalizePreferenceValue(
   plain.preference ||
    plain.mealTypePreference ||
    "VEG",
  ),
  bannerImage: plain.bannerImage || "",
  cuisines:
   plain.cuisines || plain.cuisineTypes || [],
  kitchenName:
   plain.kitchenName || plain.name || "Kitchen",
 };
};
...
const mongoUri =
 ...
 "mongodb://127.0.0.1:27017/homefeast";
mongoose
 .connect(mongoUri)
 .then(() =>
  console.log(
...
app.get("/api/users", async (req, res) => {
 try {
  ...
  res
   .status(500)
   .json({
    error: "Failed to fetch users collection",
   });
 }
});
...
app.delete("/api/users/:id", async (req, res) => {
 try {
  ...
  res
   ...
   .json({
    message:
     "Server error while attempting to delete user.",
    error: error.message,
   });
 }
});
  ...
  try {
   ...
   if (!vendor) {
    return res
     .status(400)
     .json({
      message: "❌ Vendor kitchen not found.",
     });
   }
   ...
   res.status(200).json({
    message:
     "🎉 Vendor dashboard access authorized!",
    vendor: {
     id: vendor._id,
     email: vendor.email,
...
app.get("/api/cooks", async (req, res) => {
 try {
  ...
  res
   .status(200)
   .json(cooks.map(normalizeKitchenData));
 } catch (error) {
  console.error(error);
  res
   .status(500)
   .json({
    message: "Server error while fetching cooks.",
   });
 }
});
...
app.post(
 ...
 async (req, res) => {
  try {
   ...
   let vendorExists = await Cook.findOne({
    email,
   });
   if (vendorExists) {
    return res
     ...
     .json({
      message:
       "❌ This kitchen email is already registered!",
     });
   }
   ...
   res
    ...
    .json({
     message:
      "Server error during vendor signup.",
    });
  }
 },
);
...
app.get("/api/disputes", async (req, res) => {
 try {
  const disputes = await Dispute.find({}).sort({
   createdAt: -1,
  });
  ...
  res
   ...
   .json({
    message:
     "Server error while fetching disputes.",
   });
 }
});
 ...
 try {
  ...
  if (!name || !email || !issue) {
   return res
    ...
    .json({
     message:
      "Name, email, and issue description are required.",
    });
  }
...
app.delete(
 ...
 async (req, res) => {
  res
   .status(200)
   .json({ message: "Cuisine removed." });
 },
);

app.patch(
 "/api/cooks/:id/approve",
 async (req, res) => {
  try {
   ...
   if (!cook) {
    return res
     .status(404)
     .json({ message: "Cook not found" });
   }
   res
    .status(200)
    .json({
     message: "Cook approval updated.",
     cook,
    });
  } catch (error) {
   console.error(error);
   res
    .status(500)
    .json({ message: "Failed to approve cook." });
  }
 },
);
 ...
 async (req, res) => {
  try {
   ...
   if (!updatedDispute) {
    return res
     .status(404)
     .json({ message: "Dispute not found." });
   }
...
app.get("/api/kitchens/:id", async (req, res) => {
 try {
  const kitchen = await Cook.findById(
   req.params.id,
  );
  if (!kitchen) {
   return res
    .status(404)
    .json({ message: "Kitchen not found" });
  }
  res
   .status(200)
   .json(normalizeKitchenData(kitchen));
 } catch (error) {
  console.error(error);
  res
   .status(500)
   .json({
    message:
     "Server error fetching kitchen profile",
   });
 }
});
...
app.put("/api/kitchens/:id", async (req, res) => {
 try {
  ...
  const kitchen = await Cook.findByIdAndUpdate(
   ...
   {
    kitchenName:
     payload.name || payload.kitchenName,
    description: payload.description,
    serviceArea:
     payload.location || payload.serviceArea,
    mealTypePreference: normalizePreferenceValue(
     payload.preference ||
      payload.mealTypePreference,
    ),
  ...
  res
   ...
   .json({
    message:
     "Server error updating kitchen profile",
   });
 }
});

app.patch(
 "/api/kitchens/:id/rating",
 async (req, res) => {
  try {
   ...
   const clampedRating = Math.max(
    0,
    Math.min(5, Number(nextRating.toFixed(1))),
   );
   ...
   res
    ...
    .json({
     message:
      "Server error updating kitchen rating",
    });
  }
 },
);
...
app.get("/api/menu", async (req, res) => {
 try {
  ...
  res
   .status(200)
   .json(menuItems.map(normalizeMenuItem));
 } catch (error) {
  console.error(error);
  res
   .status(500)
 ...
 try {
  ...
  if (!name || !price || !kitchenId) {
   return res
    ...
    .json({
     message:
      "Missing required fields: name, price, kitchenId",
    });
  }
  ...
  res
   .status(201)
   .json(normalizeMenuItem(newMenuItem));
 } catch (error) {
  console.error(error);
  res
   .status(500)
 ...
 try {
  ...
  if (!payload.userEmail) {
   return res
    ...
    .json({
     message:
      "User email is required to place an order.",
    });
  }
  ...
  const newOrder = new Order({
   ...
   deliveryOption:
    payload.deliveryOption || "homeDelivery",
 ...
 async (req, res) => {
  try {
   ...
   if (vendorReview !== undefined) {
    ...
    update.reviewSentAt = vendorReview
     ? new Date()
     : null;
   }
